...
const formatDate = function (date) {
  if (!date) return ''
    ...
    const utcDate = (0,date_fns_tz__WEBPACK_IMPORTED_MODULE_2__.utcToZonedTime)(dateInput)
    const formattedDate = (0,date_fns_tz__WEBPACK_IMPORTED_MODULE_2__.format)(utcDate, 'MM/dd/yyyy')
...
function createTodo (
  ...
  const createdOnDate = formatDate(createdOn)
...
const todoInput = () => {
  openBtn.addEventListener('click', function () {
    // inputModal.toggleAttribute('hidden')
...
function generateProjectList () {
  ...
  for (const project in _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList) {
    ...
    const editIcon = createIcon(_edit_png__WEBPACK_IMPORTED_MODULE_4__)
    editIcon.addEventListener('click', () =>
      handleEditProject(project, listItem)
    )

    const deleteIcon = createIcon(_recycle_bin_png__WEBPACK_IMPORTED_MODULE_3__)
...
function handleEditProject (project, listItem) {
  ...
  input.addEventListener('change', () => {
    ...
    const newProject = _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList[project]
    delete _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList[project]
    _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList[newProjectName] = newProject
...
function createRow (todo, project) {
  ...
  const completedCell = createCompletedCell(todo, project)
...
function createCompletedCell (todo, project) {
  const cell = document.createElement('td')
  cell.setAttribute('class', 'completed')
  const input = createCompletedInput(todo, project)
  cell.appendChild(input)
  return cell
}

function createCompletedInput (todo, project) {
  const input = document.createElement('input')
  input.setAttribute('type', 'checkbox')
  input.checked = todo.isCompleted
  input.addEventListener('change', (event) => {
    markAsCompleted(todo, input, project)
    event.stopPropagation()
  })
  return input
}
...
function renderPopup (todo, project) {
  renderTodoInfo(todo)
  addCompletedListener(todo, project)
  addDeleteListener(todo)
  addCloseListener(project)

  const addInputEventListener = (element, type, prop) => {
    element.addEventListener('click', () => {
      ...
      input.addEventListener('change', () => {
        if (type === 'date') {
          const date = (0,_createTodo__WEBPACK_IMPORTED_MODULE_1__.formatDate)(input.value)
          todo[prop] = date
        } else {
          todo[prop] = input.value
        }
        (0,_localStorage__WEBPACK_IMPORTED_MODULE_2__.saveToLocalStorage)(_createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList)
  ...
  addInputEventListener(dispTodoName, 'text', 'name')
  addInputEventListener(dispTodoDetails, 'text', 'details')
  addInputEventListener(dispTodoDueDate, 'date', 'dueDate')
...
function renderTodoInfo (todo) {
  dispTodoProject.textContent = todo.project
  dispTodoName.textContent = todo.name
  dispTodoDetails.textContent = todo.details
  dispTodoDueDate.textContent = todo.dueDate
  dispTodoPriority.textContent = todo.priority
  dispTodoCreatedOn.textContent = todo.createdOn
}
function addCompletedListener (todo, project) {
  const input = document.getElementById('todoCompleted')
  input.addEventListener('change', (event) => {
    markAsCompleted(todo, input, project)
    event.stopPropagation()
  })
...
function addDeleteListener (todo) {
  dispTodoDelete.addEventListener('click', function () {
    todo.toDelete = true
    deleteTodo(todo)
  })
}
...
function deleteTodo (todo) {
  const project = _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList[todo.project]
  ...
  if (project === _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList.Completed) {
    generateTable(_createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList.Completed)
  } else if (project === getAllTodos()) {
    generateTable(getAllTodos())
  } else {
    generateTable(_createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList[todo.project])
    popupModal.style.display = 'none'
  }
}

function addCloseListener (project) {
...
function markAsCompleted (todo, input, project) {
  ...
  if (indexC === -1 && todo.isCompleted) {
    addToCompleted(todo, _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList.Completed)
    removeFromProject(todo, project)
  } else if (indexP === -1 && !todo.isCompleted) {
    addToProject(todo, project)
    removeFromProject(todo, _createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList.Completed)
  }
  (0,_localStorage__WEBPACK_IMPORTED_MODULE_2__.saveToLocalStorage)(_createTodo__WEBPACK_IMPORTED_MODULE_1__.projectList)
  generateTable(project)
}
function addToCompleted (todo, project) {
  project.push(todo)
}
